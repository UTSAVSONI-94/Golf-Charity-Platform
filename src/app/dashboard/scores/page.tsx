  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-neutral-400">Submit your Stableford performance metrics (1-45). Only your latest 5 entries are kept — the rolling window ensures fairness in the draw algorithm.</p>
      </div>
      ...
      {/* Rolling 5 Status Banner */}
      <div className={`p-4 rounded-xl flex items-center gap-3 ${isFull ? 'bg-amber-500/10 border border-amber-500/20 text-amber-400' : 'bg-blue-500/10 border border-blue-500/20 text-blue-400'}`}>
        <Info className="w-5 h-5 shrink-0" />
        <p className="text-sm font-medium">
          {isFull 
            ? 'You have 5/5 entries. Adding a new score will automatically replace your oldest entry.'
            : `You have ${scoreCount}/5 entries. Add ${5 - scoreCount} more to qualify for the monthly draw.`
          }
        </p>
      </div>
      ...
      <form action={addScore} className="glass rounded-3xl p-6 flex flex-col sm:flex-row items-end gap-4 border border-neutral-800 bg-neutral-900/40">
        <div className="flex-1 w-full">
          <label className="block text-sm font-medium text-neutral-300 mb-2">Stableford Score (1-45)</label>
          <input 
            type="number" 
            name="score"
            min="1" max="45"
        ...
        <div className="flex-1 w-full">
          <label className="block text-sm font-medium text-neutral-300 mb-2">Round Date</label>
          <input 
            type="date" 
            name="date"
            max={today}
        ...
        >
          <Plus className="w-5 h-5" /> {isFull ? 'Replace Oldest' : 'Add Entry'}
        </button>
      </form>

      {/* Score List */}
      <div className="space-y-4">
        <h2 className="text-xl font-bold text-neutral-200">Rolling 5 Entries ({scoreCount}/5)</h2>
        {!scores || scores.length === 0 ? (
          <div className="glass rounded-3xl p-10 text-center text-neutral-500 border border-neutral-800">
            No entries submitted yet. Add your first Stableford score above to begin!
          </div>
        ) : (
            ...
            {scores.map((s, i) => (
              ...
              >
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <p className="font-semibold text-white text-lg">Entry #{scoreCount - i}</p>
                    <p className="text-neutral-400 text-sm">Round played on {s.date}</p>
                  </div>
                </div>
